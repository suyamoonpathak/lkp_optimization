...
62.5$\times$ (1001~$\to$~16) and cuts wall-clock time by 23\%; the
INSERT\_RANGE numbers are nearly identical. Independent
bare-metal runs by our collaborator confirm both patches: 48\%
wall-clock speedup for xattr (same 64$\times$ commit reduction);
43--44\% wall-clock speedup for fallocate (same 62.5$\times$ commit
reduction). The two patches touch disjoint files and compose without
conflict. They introduce no new regressions in any xfstests run we
performed; all six of our custom crash-recovery tests pass. The
four-attempt process also yielded lessons about measurement
methodology that the report documents.
\end{abstract}
...
We went through four candidate patches in total. The first two,
while technically correct, could not be measured above the noise
floor on our VM. The third closes a real gap in ext4's fast-commit
feature that touches 100\% of \texttt{setxattr} operations, and
gives a speedup large enough to rise above any noise. The fourth
followed the same recipe on a different fast-commit fallback ---
\texttt{fallocate(COLLAPSE\_RANGE)} and
\texttt{fallocate(INSERT\_RANGE)} --- and produced an
architecturally identical 62.5$\times$ commit-count reduction.

We believe the full four-attempt arc is the valuable part of this
project, not just the final patches. The two failures taught us to
pick optimization targets by hit rate and not just code elegance,
and the third success let us formalise that rule into a
``measure before you code'' characterization step that we used to
pick the fourth target. This report therefore documents all four
honestly, including what went wrong and how it shaped the next
attempt.
...
After C3 succeeded, we asked whether any other fast-commit
fallback in ext4 has the same shape: 100\% hit rate on a specific
operation, simple enough to fix with a small patch, distinguishable
on a microbench. We built three small benchmarks and measured them
on the C3 kernel before writing any code (Section~7.1). The winner
was the \texttt{FALLOC\_RANGE} fallback, and that became
Candidate~4.

This ``characterize before you patch'' step is, in our view, the
methodology lesson of the project. C1 and C2 each cost a week
because we picked targets by code elegance; C4 took roughly two
days because we picked it by measured signal.
...
Milan re-ran the same benchmark on his bare-metal machine: 11th~Gen
Intel Core i3-1115G4, 4~cores, 7.5~GiB RAM, real SATA SSD. Three
repeats of each configuration. Baseline kernel: his already-installed
...
total. C3 went from 27\% (VM) to 48\% (Milan). C4 went from 23--26\%
(VM) to 42--44\% (Milan). The two-step factor between VM and
bare-metal is consistent across both patches, which is what we would
expect if the underlying source of the gap is the same.
...
wall-time gain for the two modes. On Milan's bare-metal box the
same 62.5$\times$ commit reduction reproduces byte-identically and
the wall-time gain nearly doubles to 42--44\%. Three crash-recovery
...
benchmarks that confirmed both Candidate~3 (48\% wall-time
speedup, same 64$\times$ commit reduction as our VM) and
Candidate~4 (43--44\% wall-time speedup, same 62.5$\times$ commit
reduction as our VM). Thanks to Sahil for being ready to run the
bare-metal benchmarks as a second cross-check.
...
# C4 bare-metal (Milan)
cat our_optimization/eval_results_c4/milan/BASELINE_6.1.4-C3Patch/collapse_summary.txt
cat our_optimization/eval_results_c4/milan/BASELINE_6.1.4-C3Patch/insert_summary.txt
cat our_optimization/eval_results_c4/milan/PATCHED_C4_6.1.4-C3C4Patch/collapse_summary.txt
cat our_optimization/eval_results_c4/milan/PATCHED_C4_6.1.4-C3C4Patch/insert_summary.txt
\end{lstlisting}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textit{How to access.}
        \pth{our_optimization/eval_results_c3/<contributor>/*/xattr_loop.txt}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textit{How to run.} Follow
        \texttt{INSTRUCTIONS\_MILAN\_C4.md} on a bare-metal
        ...
        \texttt{eval\_milan\_c4.sh} once on each kernel.
  \item \textit{Estimated runtime.} 35--45~minutes per kernel,
        mostly kernel build; the bench itself is under a minute.
  ...
  \item \textit{Recorded result (Milan, this run).} COLLAPSE:
  ...
  \item \textit{How to access.} The two summary directories are
        \pth{our_optimization/eval_results_c4/milan/BASELINE_6.1.4-C3Patch/}
        and
        \pth{our_optimization/eval_results_c4/milan/PATCHED_C4_6.1.4-C3C4Patch/}.
\end{itemize}
