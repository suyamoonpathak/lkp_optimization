# Candidate 3 Evaluation Instructions (Sahil)

Hi Sahil — **this is the candidate that worked** on our VM: 64× fewer
...
```bash
cd ~/jbd2-project
git checkout -b results-c3/sahil
```
...
# in REPO
cd ~/jbd2-project
sudo bash our_optimization/eval_sahil_c3.sh
```

Runtime ~5 min. Auto-detects STOCK vs PATCHED by scanning your kernel
source tree for `bool touched_block` (absent = stock).

Results save to `our_optimization/eval_results_c3/sahil/STOCK_<kernel>/`.

Commit and push right away:
```bash
# in REPO
cd ~/jbd2-project
git add our_optimization/eval_results_c3/sahil/
git commit -m "sahil: c3 stock baseline"
git push -u origin results-c3/sahil
```
...
Results save to `our_optimization/eval_results_c3/sahil/PATCHED_<kernel>/`.
...
cd ~/jbd2-project
git add our_optimization/eval_results_c3/sahil/
git commit -m "sahil: c3 patched results"
git push origin results-c3/sahil
```

Open a PR from `results-c3/sahil` → `master`, or just ping Suyamoon.
