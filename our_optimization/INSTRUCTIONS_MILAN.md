# JBD2 Fast Commit Optimization — Evaluation Instructions (Milan)

Hi Milan — please run these steps on your bare-metal laptop that has
...
# in REPO
cd ~/jbd2-project
sudo bash our_optimization/eval_milan.sh
```

This takes ~15 minutes. Results are saved to
`our_optimization/eval_results/milan/STOCK_<kernel>/`.
...
# in REPO
cd ~/jbd2-project
git add our_optimization/eval_results/milan/
git commit -m "milan: stock baseline results"
git push -u origin results/milan
```
...
Results go to `our_optimization/eval_results/milan/PATCHED_<kernel>/`.
...
cd ~/jbd2-project
git add our_optimization/eval_results/milan/
git commit -m "milan: patched kernel results"
git push origin results/milan
```

Either open a pull request from `results/milan` to `master`, or just let
