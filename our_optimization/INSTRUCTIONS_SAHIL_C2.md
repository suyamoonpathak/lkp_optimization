# Candidate 2 Evaluation Instructions (Sahil)

Hi Sahil — this is a **new patch** (Candidate 2), separate from the earlier
...
```bash
cd ~/jbd2-project
git checkout -b results-c2/sahil
```
...
# in REPO
cd ~/jbd2-project
sudo bash our_optimization/eval_sahil_c2.sh
```

Runtime: ~20 min. Results are saved to
`our_optimization/eval_results_c2/sahil/STOCK_<kernel>/`.
...
# in REPO
cd ~/jbd2-project
git add our_optimization/eval_results_c2/sahil/
git commit -m "sahil: c2 stock baseline"
git push -u origin results-c2/sahil
```
...
Results go to `our_optimization/eval_results_c2/sahil/PATCHED_<kernel>/`.
...
cd ~/jbd2-project
git add our_optimization/eval_results_c2/sahil/
git commit -m "sahil: c2 patched results"
git push origin results-c2/sahil
```

Then either open a pull request from `results-c2/sahil` to `master`, or
