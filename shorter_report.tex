...
The full patch is 108 lines
(\pth{our_optimization/fc-inline-xattr.patch}): \texttt{+9/-0} in
\texttt{fast\_commit.c} (the new conditional with its comment),
and \texttt{+27/-4} in \texttt{xattr.c} (the
...
\begin{itemize}[leftmargin=*]
  \item Three crash-recovery scripts
        ...
        odd ones remain (\textbf{PASS}). Test~C: a 668-byte
        xattr value (well over the inline limit so the xattr
        block path is used), lazy unmount, remount, full value
        recovered (\textbf{PASS}, confirming the fallback branch
        still runs the full commit when needed).
  \item xfstests subset
        ...
        Logs at \pth{our_optimization/xfstests_c3/}.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item Same xfstests subset as C3. Five of the six runnable
        ...
        to stock and to the C3 kernel. \texttt{generic/062, 337,
        454} (the xattr-heavy tests) all pass, confirming C4
        does not regress C3's work. Logs at
